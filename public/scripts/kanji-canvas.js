...
//   KanjiCanvas.strokeColors = ['#706D6D'];
  KanjiCanvas.strokeColors = ['#706D6D'];
  let predictions = [];
  ...
  KanjiCanvas.init = function (id) {
    ...
    KanjiCanvas["canvas_" + id].dataset.strokeNumbers = 'false'; //disables stroke numbering drawing
  ...
  KanjiCanvas.draw = function (id, color) {
    ...
    KanjiCanvas["ctx_" + id].strokeStyle = color ? color : "#000000";
  ...
  KanjiCanvas.deleteLast = function (id) {
	...
	// console.log(KanjiCanvas["recordedPattern_" + id])
	...
	predictions = KanjiCanvas.recognize('drawCanvas');
  ...
  KanjiCanvas.findxy = function (res, e, id) {
	...
	predictions = KanjiCanvas.recognize('drawCanvas');
	// console.log(localStorage.getItem("predictions"))
	//   document.getElementById("results").innerHTML = predictions;
    }
  ...
  KanjiCanvas.redraw = function (id) {
    ...
    for(var i = 0;i<KanjiCanvas["recordedPattern_" + id].length;i++) {
      ...
      for(var j = 0; j<stroke_i.length-1;j++) {
        ...
        KanjiCanvas["currX_" + id] = stroke_i[j+1][0];
        KanjiCanvas["currY_" + id] = stroke_i[j+1][1];
        KanjiCanvas.draw(id, KanjiCanvas.strokeColors[0]);
      }
    }
